...
>
> **Desktop note:** feature pin **3.11** / **2026-08-13**; desktop CLI observed **3.15.19**. Cloud Agent **Builds** (2026-08-13) warm the install snapshot — enable on the environment's Builds tab. Optional desktop `workspaceOpen` hook can load workspace plugin paths. **Debugging tip (3.11):** open a side chat (`/side` / `/btw`) to inspect MCP
