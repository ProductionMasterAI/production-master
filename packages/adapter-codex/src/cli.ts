#!/usr/bin/env node
/**
 * cli — the Codex thin-client binary. Mirrors adapter-claude-code/cli.ts:
 * it composes the single `createPluginRuntime` composition root with the
 * `CodexHostAdapter`, backed by terminal sinks, and never runs investigation
 * logic locally — the run executes entirely server-side (AD-7 runtime clause /
 * dev#104).
 *
 * Codex supports two distinct invocation shapes, both served by this one
 * binary:
 *
 ...
 *      shelling out from a script or a Codex "run command" tool call.
 *
 *   2. `mcp` — a persistent JSON-RPC-over-stdio MCP tool server. This is the
 *      shape `.codex/config.toml`'s `[mcp_servers.production-master]` block
 *      spawns: Codex starts `node cli.js mcp` once and keeps it alive,
 *      speaking newline-delimited JSON-RPC 2.0 (`initialize`, `tools/list`,
 ...
 *      command uses — no protocol logic is duplicated, and mutations still
 *      go through the host's confirmation sink before anything reaches the
...
/** Flatten a `CodexPanelState`'s commands into plain terminal lines. */
...
/**
 * Terminal-backed Codex sinks. Everything the adapter needs is a real side
 * effect on the current terminal; nothing here logs token material.
 */
export class TerminalSinks implements CodexHostSinks {
  ...
  renderPanel(state: CodexPanelState): void {
    // Codex's native surface is a webview panel; on a raw terminal we paint a
  ...
  registerMcpServer(_cfg: McpServerConfig): void {
    // Codex's MCP registration is a static declaration in .codex/config.toml
 ...
 * (`initialize`, `tools/list`, `tools/call`) for Codex to call the
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  const runtime = createPluginRuntime({
    host: new CodexHostAdapter(new TerminalSinks()),
    config,
    deps,
  });
