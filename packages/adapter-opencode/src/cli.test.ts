...
describe('TerminalSinks', () => {
  ...
  it('paints the TUI-panel block on the err stream', () => {
    const out = capture();
    const err = capture();
    const sinks = new TerminalSinks(out.sink, err.sink, false);
    sinks.renderTuiPanel({
  ...
  it('registerMcpServer never leaks the session JWT (no-op, static opencode.json registration)', () => {
    const out = capture();
    const err = capture();
    const sinks = new TerminalSinks(out.sink, err.sink, false);
