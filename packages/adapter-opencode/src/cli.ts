#!/usr/bin/env node
/**
 * cli — the OpenCode thin-client binary. Mirrors adapter-codex/cli.ts: it
 * composes the single `createPluginRuntime` composition root with the
 * `OpenCodeHostAdapter`, backed by terminal sinks, and never runs
 * investigation logic locally — the run executes entirely server-side (AD-7
 * runtime clause / dev#104).
 *
 * OpenCode is a terminal-native coding agent. It supports two distinct
 * invocation shapes, both served by this one binary:
 *
 ...
 *      shelling out from a script or an OpenCode "run command" tool call.
 *
 *   2. `mcp` — a persistent JSON-RPC-over-stdio MCP tool server. This is the
 *      shape `opencode.json`'s `mcp.production-master` (a `type: "local"`
 *      entry) block spawns: OpenCode starts `node cli.js mcp` once and keeps
 *      it alive, speaking newline-delimited JSON-RPC 2.0 (`initialize`,
 *      `tools/list`, `tools/call`) so its own agent can call the
 *      `investigation.*` tool surface directly, without shelling out per call.
 *      Every `tools/call` forwards into the SAME `runtime.update()` the
 *      direct-dispatch `update` command uses — no protocol logic is
 *      duplicated, and mutations still go through the host's confirmation sink
 *      before anything reaches the service. `login`/`investigate`/`connect`
 *      are not exposed as MCP tools: they stream a run, which does not fit the
 *      single request/response `tools/call` shape; a user still runs
 *      `cli.js login` once beforehand.
 *
...
/** Flatten an `OpenCodeTuiPanelState`'s commands into plain terminal lines. */
...
/**
 * Terminal-backed OpenCode sinks. Everything the adapter needs is a real side
 * effect on the current terminal; nothing here logs token material.
 */
export class TerminalSinks implements OpenCodeHostSinks {
  ...
  renderTuiPanel(state: OpenCodeTuiPanelState): void {
    // OpenCode's native surface is a TUI panel; on a raw terminal we paint a
    // refreshing block — clear the previous block (TTY only) then reprint.
  ...
  registerMcpServer(_cfg: McpServerConfig): void {
    // OpenCode's MCP registration is a static declaration in opencode.json's
    // `mcp` map (the persistent `node cli.js mcp` subprocess) — it is not
    // something a running process hands to the host dynamically per session.
    // This per-investigation grant is consumed internally by this same
    // process's McpTools / HttpMcpToolTransport (used by the direct-dispatch
    // `update` command and by the `mcp` subcommand's tools/call handler). No-op
    // here, and never log the endpoint or session JWT.
  }
}
 ...
 * (`initialize`, `tools/list`, `tools/call`) for OpenCode to call the
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  const runtime = createPluginRuntime({
    host: new OpenCodeHostAdapter(new TerminalSinks()),
    config,
    deps,
  });
